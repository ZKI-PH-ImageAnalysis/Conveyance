...
class Evaluator():
    def __init__(self, data_loader, logger, writer, device, pass_model_to_loss, num_classes, loss_name=None) -> None:
        self.device = device
    ...
    def update_meters(self, logits, labels, loss):
        if self.loss_name in ['alpha_dl']:
            filtered_logits = logits[:, :self.num_classes]
